...
suite('collection', () => {
    ...
    test('sort throws exception', () => {
        const simple = collection([0, 1, 2, 3, 4, 5]);
        assert.throwsMatching(/Cannot sort collections, use sortedView instead/, () => simple.sort((a, b) => b - a));
    });
});
suite('mapView', () => {
...
suite('sortedView', () => {
    test('produces a sorted view', () => {
        const phrases = collection(['hi', 'hello', 'howdy'], 'phrases');
        const sortedPhrases = phrases.sortedView((a, b) => a < b ? -1 : a > b ? 1 : 0);
    ...
    test('handles push and unshift', () => {
        ...
        flush();
        assert.deepEqual(['a', 'green', 'robot', 'throws', 'violently'], sortedPhrases);
        release(sortedPhrases);
    });
    test('handles pop and shift', () => {
        const phrases = collection(['quick', 'brown', 'fox', 'jumped', 'over', 'the', 'lazy', 'dog'], 'phrases');
        const sortedPhrases = phrases.sortedView((a, b) => a < b ? -1 : a > b ? 1 : 0);
        retain(sortedPhrases);
        phrases.pop(); // dog
        phrases.shift(); // quick
        flush();
        assert.deepEqual(['brown', 'fox', 'jumped', 'lazy', 'over', 'the'], sortedPhrases);
        release(sortedPhrases);
    });
    test('handles splice', () => {
        const phrases = collection(['quick', 'brown', 'fox', 'jumped', 'over', 'the', 'lazy', 'dog'], 'phrases');
        ...
        flush();
        assert.deepEqual(['bear', 'brown', 'dog', 'lazy', 'quick', 'stalked', 'the'], sortedPhrases);
        release(sortedPhrases);
    });
    test('handles assignment', () => {
        ...
        phrases[2] = 'tank';
        flush();
        assert.deepEqual(['brown', 'dog', 'jumped', 'lazy', 'over', 'quick', 'tank', 'the'], sortedPhrases);
        release(sortedPhrases);
    });
    test('recalculates only on flush', () => {
        ...
        retain(sortedPhrases);
        phrases[2] = 'tank';
        assert.deepEqual(['brown', 'dog', 'fox', 'jumped', 'lazy', 'over', 'quick', 'the'], sortedPhrases);
        flush();
        assert.deepEqual(['brown', 'dog', 'jumped', 'lazy', 'over', 'quick', 'tank', 'the'], sortedPhrases);
        release(sortedPhrases);
    });
...
suite('flatMapView', () => {
    test('produces a filtered view', () => {
        const numbers = collection([1, 2, 3, 4, 5, 6], 'numbers');
        const evenDupedNumbers = numbers.flatMapView((num) => num % 2 === 0 ? [num, num] : []);
