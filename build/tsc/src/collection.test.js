...
suite('mapView', () => {
    ...
    test('handles moveSlice, not recalculating moved items', () => {
        const phrases = collection(['one', 'two', 'three', 'four', 'five'], 'phrases');
        let prefix = '';
        const exclaimations = phrases.mapView((phrase) => `${prefix}${phrase}!`, 'exclaimations');
        retain(exclaimations);
        prefix = 'new:';
        ...
        flush();
        assert.deepEqual(['one!', 'four!', 'two!', 'three!', 'five!'], exclaimations);
        release(exclaimations);
    });
...
suite('flatMapView', () => {
    ...
    test('handles moveSlice', () => {
        const numbers = collection([3, 4, 5, 6, 7], 'numbers');
        const evenDupedNumbers = numbers.flatMapView((num) => num % 2 === 0 ? [num, num] : [num]);
        retain(evenDupedNumbers);
