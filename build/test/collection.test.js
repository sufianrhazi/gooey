...
`;
  ...
  suite("collection", () => {
    test("reads feel like reads", () => {
      ...
      assert2.is(simple[0], 0);
      assert2.is(simple[1], 1);
      assert2.is(simple[2], 2);
    });
    test("deepEquality works as expected", () => {
      const simple = collection([0, 1, 2]);
      assert2.deepEqual(simple, [0, 1, 2]);
    });
    test("writes are written", () => {
      const simple = collection([0, 1, 2]);
      simple[1] = 5;
      assert2.is(simple[1], 5);
    });
    test("assigning past length extends length", () => {
      const simple = collection([0, 1, 2]);
      assert2.is(simple.length, 3);
      simple[5] = 5;
      assert2.deepEqual([0, 1, 2, void 0, void 0, 5], simple);
    });
    test("extending length extends length", () => {
      const simple = collection([0, 1, 2]);
      simple.length = 5;
      const items = simple.slice();
      assert2.deepEqual([0, 1, 2, void 0, void 0], items);
    });
    test("reducing length works as expected", () => {
      const simple = collection([0, 1, 2]);
      simple.length = 1;
      const items = simple.slice();
      assert2.deepEqual([0], items);
      assert2.is(simple[0], 0);
      assert2.is(simple[1], void 0);
      assert2.is(simple[2], void 0);
    });
    test("reject is an in-place filter", () => {
      const simple = collection([0, 1, 2, 3, 4, 5]);
      simple.reject((n) => n % 2 === 0);
      assert2.deepEqual([1, 3, 5], simple);
    });
  });
  suite("mapView", () => {
    test("produces a mapped view", () => {
      ...
      assert2.deepEqual(["hi!", "hello!", "howdy!"], exclaimations);
    ...
    test("mapView handles push, only recalculating new items", () => {
      ...
      phrases.push("cool");
      flush();
      assert2.deepEqual(["hi!", "hello!", "howdy!", "new:cool!"], exclaimations);
      release(exclaimations);
    });
    test("mapView handles pop, not recalculating anything", () => {
      ...
      phrases.pop();
      flush();
      assert2.deepEqual(["hi!", "hello!"], exclaimations);
      release(exclaimations);
    });
    test("mapView handles unshift, only recalculating new items", () => {
      ...
      flush();
      assert2.deepEqual(["new:cool!", "hi!", "hello!", "howdy!"], exclaimations);
      release(exclaimations);
    });
    test("mapView handles shift, not recalculating anything", () => {
      ...
      phrases.shift();
      flush();
      assert2.deepEqual(["hello!", "howdy!"], exclaimations);
      release(exclaimations);
    });
    test("mapView handles splice, only recalculating new items", () => {
      ...
      flush();
      assert2.deepEqual(["hi!", "new:wow!", "new:neat!", "new:fun!", "howdy!"], exclaimations);
      release(exclaimations);
    });
    test("mapView handles writes, only recalculating new items", () => {
      ...
      phrases[1] = "wow";
      flush();
      assert2.deepEqual(["hi!", "new:wow!", "toAlsoRemove!", "howdy!"], exclaimations);
      phrases[2] = "neat";
      flush();
      assert2.deepEqual(["hi!", "new:wow!", "new:neat!", "howdy!"], exclaimations);
      release(exclaimations);
    });
    test("mapView recalculates writes only upon flush", () => {
      ...
      prefix = "new:";
      flush();
      assert2.deepEqual(["new:new beginning!", "new:new middle!", "new:new end!"], exclaimations);
      release(exclaimations);
    });
  });
})();
