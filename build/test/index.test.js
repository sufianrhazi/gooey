  ...
  suite("behavior", () => {
    ...
    function setUp() {
      ...
      const todoList = model({
        name: "Shopping",
        items: collection([model0, model1])
      });
      ...
      const makeTodoListRenderer = (todoList2) => {
        return calc(() => {
          renders.push("list");
          const lines = [`${todoList2.name}:`];
    ...
    test("initial render renders tree", () => {
      ...
      assert2.is(app(), "Shopping:\n[ ] apples\n[ ] bananas");
      assert2.deepEqual(renders, ["list", "item:model0", "item:model1"]);
      model0.task = "what";
      flush();
      assert2.is(app(), "Shopping:\n[ ] what\n[ ] bananas");
    });
    ...
    test("change multiple dependencies", () => {
      ...
      flush();
      assert2.deepEqual(renders.slice(0, 3), [
        "list",
        "item:model0",
        "item:model1"
      ]);
      const postFlushRenders = renders.slice(3);
      assert2.arrayIncludes(postFlushRenders, "item:model0");
      assert2.arrayIncludes(postFlushRenders, "item:model1");
      assert2.arrayIncludes(postFlushRenders, "list");
      assert2.lessThan(postFlushRenders.indexOf("item:model0"), postFlushRenders.indexOf("list"));
      assert2.lessThan(postFlushRenders.indexOf("item:model1"), postFlushRenders.indexOf("list"));
      assert2.is(app(), "Shopping:\n[x] apples\n[ ] cherries");
    });
    ...
    test("fancy array stuff like splicing works", () => {
      ...
      todoList.items = collection([model3, model0, model1, model2]);
      app();
  ...
  suite("invariants", () => {
    suite("models feel like objects", () => {
      ...
      test("reads feel like reads", (ctx) => {
        assert2.is(ctx.model.foo, 3);
        assert2.deepEqual(ctx.model.bar, { hello: "world" });
      });
      test("deepEquality works as expected", (ctx) => {
        assert2.deepEqual(ctx.model, {
          foo: 3,
          bar: { hello: "world" }
        });
      });
      test("writes are written", (ctx) => {
        ctx.model.foo = 4;
        assert2.is(ctx.model.foo, 4);
      });
      test("keys are correct", (ctx) => {
        const keys2 = Object.keys(ctx.model);
        assert2.arrayIncludes(keys2, "foo");
        assert2.arrayIncludes(keys2, "bar");
      });
    });
  });
})();
