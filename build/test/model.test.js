...
`;
  ...
  suite("model", () => {
    test("reads feel like reads", () => {
      ...
      assert2.is(simple.foo, 3);
      assert2.deepEqual(simple.bar, { hello: "world" });
    });
    ...
    test("writes are written", () => {
      ...
      simple.foo = 4;
      assert2.is(simple.foo, 4);
    });
    test("Object.keys works as expected", () => {
      const simple = model({});
      assert2.arrayEqualsUnsorted([], Object.keys(simple));
      simple.foo = "a";
      assert2.arrayEqualsUnsorted(["foo"], Object.keys(simple));
      simple.bar = "a";
      assert2.arrayEqualsUnsorted(["foo", "bar"], Object.keys(simple));
      simple.foo = "b";
      assert2.arrayEqualsUnsorted(["foo", "bar"], Object.keys(simple));
      delete simple.bar;
      assert2.arrayEqualsUnsorted(["foo"], Object.keys(simple));
    ...
    test("model.keys produces view of keys", () => {
      const simple = model({});
      const keys2 = model.keys(simple);
      retain(keys2);
      assert2.arrayEqualsUnsorted([], keys2);
      simple.foo = "a";
      flush();
      assert2.arrayEqualsUnsorted(["foo"], keys2);
      simple.bar = "a";
      flush();
      assert2.arrayEqualsUnsorted(["foo", "bar"], keys2);
      simple.foo = "b";
      flush();
      assert2.arrayEqualsUnsorted(["foo", "bar"], keys2);
      delete simple.bar;
